...
const REALSIZE = (() => {
  ...
  function drawCigar(v) {
    ...
    box.innerHTML = `
      ...
      <div class="rs-caption">${T("<b>{ja}</b>（{en}）実物大 — 長さ約{len}mm × 直径約{dia}mm（RG {rg}）", { ja: esc(I18N.isEn ? v.en : v.ja), en: esc(I18N.isEn ? v.ja : v.en), len: Math.round(lenMm), dia: rgMm.toFixed(1), rg: esc(v.rg) })}</div>
      <div class="photo-hint">${T("画面が小さい場合は横にスクロールできます。手持ちの葉巻と重ねて比べてみてください。")}</div>`;
  }
  ...
  function init() {
    ...
    root.innerHTML = `
      <h3>${T("実物大で見る（画面ものさし）")}</h3>
      <p class="prose" style="margin-bottom:12px">${T("選んだビトラを<b>実物大</b>で画面に表示します。正確に表示するには、最初に一度だけ「ものさし合わせ」をしてください。")}</p>
      <details class="acc rs-cal">
        <summary>📐 ${T("ものさし合わせ（最初に一度だけ）")}</summary>
        <div class="acc-body">
          <p>${T("クレジットカード（またはICカード）を下の枠に<b>横向きに重ね</b>、スライダーで枠の幅をカードのサイズにピッタリ合わせてください。")}</p>
          <div class="rs-cardbox"><div class="rs-card" id="rsCard">${T("クレジットカード（横 85.6mm）")}</div></div>
          <input type="range" id="rsCal" min="2" max="10" step="0.02" style="width:100%">
          <div class="photo-hint">${T("現在の設定：")}<span id="rsCalVal"></span>${T("（この端末に記憶されます）")}</div>
        </div>
      </details>
      <div class="rs-picker">${vitolas.map((v, i) => `<button type="button" class="chip rs-pick${i === 0 ? " on" : ""}" data-rs="${i}">${esc(I18N.isEn ? v.en : v.ja)}</button>`).join("")}</div>
      <div id="rsCigar"></div>`;
