...
また、記録の「シェア →🌐 英語テキストで共有」で、感想メモを含めた記録文をAIが自然な英語に
翻訳してコピー・共有できます。

これらは同じEdge Functionの `mode: "comment"` / `mode: "translate"` を使うため、**古いバージョンの
関数をデプロイしている場合は、このリポジトリの最新の `index.ts` を貼り直して再デプロイ**して
ください。（ボタンを押して「サーバー関数の更新が必要です」と出たら、それが合図です）
